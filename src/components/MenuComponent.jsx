...
const MenuComponent = () => {
  ...
  const categoryname = hotel?.category?.name;
  ...
  useEffect(() => {
    ...
    if (categoryname === "veg") {
      ...
      axios
        .post("http://localhost:8086/api/vegmenu", hotel)
        .then((response) => {
    ...
    } else if (categoryname === "non_veg") {
      // Assuming 2 is the category ID for non-veg menu
      axios
        .post("http://localhost:8086/api/nonvegmenu", hotel)
        .then((response) => {
          setMenuItems(response.data);
        })
        .catch((error) => {
